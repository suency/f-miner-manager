...
class Star extends BaseController
{
    ...
    {
        $ssh = new SSH2('192.168.21.152:22');
        if (!$ssh->login('tiger', '123456')) {
    ...
    public function blockWin()
    {
        ...
        //phpinfo();
        /* $ssh = new SSH2('192.168.21.152');
        if (!$ssh->login('tiger', '123456')) {
            throw new \Exception('Login failed');
        }

        $re = $ssh->read('tiger@tiger:~$');
        ...
        $server   = "192.168.21.152"; // server IP/hostname of the SSH server
        $username = "tiger"; // username for the user you are connecting as on the SSH server
        $password = "123456"; // password for the user you are connecting as on the SSH server
